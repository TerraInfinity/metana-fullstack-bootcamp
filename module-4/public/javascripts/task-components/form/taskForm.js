...
import { Task } from '/javascripts/task-components/core/task.js';

// Declare jQuery variables for window global objects
const $ = window.jQuery || window.$;
const datepicker = $ ? $.fn.datepicker : null;
...
export function initializeTaskFormModal() {
    ...
    const modalContainer = document.getElementById('addTaskModalContainer'); // Get the modalContainer
    if (!modalContainer) {
    ...
    fetch('/templates/task-components/taskForm.hbs') // Load the task form from templates directory
        .then(response => {
            if (!response.ok) {
...
export function showTaskFormModal(task = null) {
    ...
    if (task) {
        console.debug('%c showTaskFormModal() task', 'color: aqua', task);
        ...
        console.debug('%c showTaskFormModal() no task provided', 'color: aqua');
    ...
    console.debug('%c showTaskFormModal() durationToggleButton listener configured', 'color: aqua');


    const datepickerEl = modal.querySelector('#datepicker');
    console.debug('%c showTaskFormModal() datepicker element', 'color: aqua', datepickerEl);
    ...
    if ($ && $.fn && $.fn.datepicker) {
        try {
            ...
            console.debug('%c showTaskFormModal() datepicker initialized', 'color: aqua');
        } catch (error) {
            console.error('%c Error initializing datepicker:', 'color: red', error);
        }
    } else {
        console.error('%c jQuery or datepicker not available. $ =', 'color: red', $);
        // Fallback to a basic date input
    ...
    console.debug('%c showTaskFormModal() clearTaskFormErrors() called', 'color: aqua');
